...
// Local lightweight fallback for Radix `Slot` so this file works without
// installing `@radix-ui/react-slot`. Replace with the official import
// if you install the package.
const Slot = ({ children }: { children?: React.ReactNode }) => <>{children}</>;

// Minimal `cva` and `VariantProps` fallback to avoid requiring
// `class-variance-authority`. This supports the basic variant maps
// used in this file. Install the package if you need full CVA features.
type VariantProps<T> = Record<string, any>;
function cva(base: string, opts?: any) {
  const variants = opts?.variants || {};
  const defaultVariants = opts?.defaultVariants || {};
  return (params: any = {}) => {
    const classes: string[] = [base];
    for (const key in variants) {
      const map = variants[key] || {};
      const val = params[key] ?? defaultVariants[key];
