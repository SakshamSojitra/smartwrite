...
// Minimal CVA-like helper to produce variant classes without external dependency
function buttonVariants({ variant = 'default', size = 'default' } = {}) {
  const base = 'relative group border text-foreground mx-auto text-center rounded-full'
  ...
  return [base, variantMap[variant] || variantMap.default, sizeMap[size] || sizeMap.default].join(' ')
}

const Button = React.forwardRef(function Button(
  { className, neon = true, size = 'default', variant = 'default', children, ...props },
  ref
) {
  return (
    <button
